...
void HttpData::handleRead() {
    ...
    do {
        ...
        LOG << "Request: " << inBuffer_;//将输入缓冲写入日志
        ...
        if (state_ == STATE_PARSE_URI) {
            ...
            if (flag ==PARSE_URI_AGAIN)
                ...
                state_ = STATE_PARSE_HEADERS;//开始解析请求头部
        }
...
AnalysisState HttpData::analysisRequest() {
    if (method_ == METHOD_POST) {}
        ...
        if (fileName_ == "favicon.ico") {
            ...
            header += "This is MongoWeb Server\r\n";
            header += "\r\n";
            outBuffer_ += header;
        ...
        int src_fd = open(fileName_.c_str(), O_RDONLY, 0);//打开文件
        if (src_fd < 0) {
            outBuffer_.clear();
            handleError(fd_, 404, "Not Found!");
            return ANALYSIS_ERROR;
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
    ...
    body_buff += "<hr><em> This is MongoWeb Server</em>\n</body></html>";
    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "This is MongoWeb Server\r\n";
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
